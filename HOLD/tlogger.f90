...
module tlogger
  ...
  integer,            public,  parameter :: tlogger_unset         = 0
  integer,            public,  parameter :: tlogger_critical      = 1
  integer,            public,  parameter :: tlogger_error         = 2
  integer,            public,  parameter :: tlogger_warning       = 3
  integer,            public,  parameter :: tlogger_info          = 4
  integer,            public,  parameter :: tlogger_debug         = 5

  integer,            private, save      :: tlogger_abort_level   = tlogger_critical
  integer,            private, save      :: tlogger_file_level    = tlogger_warning
  integer,            private, save      :: tlogger_console_level = tlogger_info
  integer,            private, save      :: tlogger_err_unit      = ERROR_UNIT
  character(len=256), private, save      :: tlogger_log_file_name = 'proxima.log'
  logical,            private, save      :: tlogging_enabled      = .true.
  logical,            private, save      :: tlogging_file_enabled = .false.
...
contains !/**                   P R O C E D U R E   S E C T I O N                       **
  ...
  subroutine tlogger_default_abort_handler( level )
    ...
    !/ -----------------------------------------------------------------------------------
    integer, intent(in) :: level !! level exit code.
    !/ -----------------------------------------------------------------------------------
  ...
  subroutine tlogger_set_abort_handler( proc, new_level )
    ...
    integer,    optional, intent(in) :: new_level !! new minimum level to generate an abort. (default: no change)
    !/ -----------------------------------------------------------------------------------
  ...
  function log_timestamp() result( ts )
    ...
    !/ -----------------------------------------------------------------------------------
    implicit none
    character(len=:), allocatable :: ts !! string of the time stamp.
    !/ -----------------------------------------------------------------------------------
    integer           :: dt(8)
...
100 format(I4,I2.2,'.',I2.2,' ',I2.2,':',I2.2,':',I2.2,'.',I3.3)
  ...
  subroutine tlogger_set( fspc, console, file, abort, unit, reset )
    ...
    integer,          optional, intent(in) :: file    !! optional integer log file level to report.
    integer,          optional, intent(in) :: abort   !! optional integer abort level to activate.
    integer,          optional, intent(in) :: unit    !! integer unit to report console messages on.
    logical,          optional, intent(in) :: reset   !! optional logical if set to true all missing arguments are reset
    !/ -----------------------------------------------------------------------------------

    if ( present(reset) ) then
       tlogger_abort_level   = tlogger_critical
       tlogger_file_level    = tlogger_warning
  ...
  end subroutine tlogger_on

  
  !/ =====================================================================================
  subroutine tlogger_off
    !/ -----------------------------------------------------------------------------------
...
110 format('[',A,'] **',A1,'** ',A)
  ...
  !/ =====================================================================================
  subroutine log_warning( msg, R8, R4, I4, I2, STR )
    !/ -----------------------------------------------------------------------------------
    ...
    if (tlogging_enabled) then
       call tlogger_write_message( tlogger_warning, msg, R8, R4, I4, I2, STR )
    end if
  ...
  subroutine log_info( msg, R8, R4, I4, I2, STR )
    !/ -----------------------------------------------------------------------------------
    !! Generate a information log message.
    !/ -----------------------------------------------------------------------------------
    implicit none
    character(len=*),               intent(in) :: msg !! message.                              
    real(dp),             optional, intent(in) :: R8  !! optional 64 bit float   display value.
